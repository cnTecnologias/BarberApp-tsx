...
export const validateAppointmentPayload = (appointment: Partial<Appointment>): string | null => {
  if (!appointment.customerId) return 'Error: Seleccioná un cliente.';
  if (!appointment.barberId) return 'Error: Asigná un barbero.';
  if (!appointment.serviceIds || appointment.serviceIds.length === 0) return 'Error: Seleccioná al menos un servicio.';
  
  const start = new Date(appointment.startTime as string);
  if (isNaN(start.getTime())) return 'Error: El horario de inicio es inválido.';
  if (start < new Date()) return 'Error: No podés agendar un turno en el pasado.';
